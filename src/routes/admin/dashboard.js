...
export async function getDashboard(request, env) {
  ...
  const [{ total_revenue }] = (
    await env.DB.prepare(
      `SELECT COALESCE(SUM(total_paise),0) AS total_revenue FROM orders WHERE status IN ('confirmed','shipped','delivered')`
    ).all()
  ).results;
  const [{ pending_orders }] = (
    await env.DB.prepare(`SELECT COUNT(*) AS pending_orders FROM orders WHERE status = 'pending'`).all()
  ).results;
